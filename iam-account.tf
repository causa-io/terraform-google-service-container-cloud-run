locals {
  service_account_email = var.service_account_email != null ? var.service_account_email : google_service_account.service[0].email
}

# The automatically-created service account for the Cloud Run service.
resource "google_service_account" "service" {
  count = var.service_account_email != null ? 0 : 1
